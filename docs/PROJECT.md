...
- кнопки Kerugi создают durable typed commands только в authenticated `running` Kerugi session, с unique ID, monotonic client sequence и clock-adjusted timestamp; они немедленно dispatch через serialized authenticated socket channel и показывают pending/accepted/rejected feedback. Server integration, reconnect physical proof и Tanbon controls остаются pending;
