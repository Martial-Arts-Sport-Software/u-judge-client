...
- кнопки Kerugi создают durable typed commands только в authenticated `running` Kerugi session, с unique ID, monotonic client sequence и clock-adjusted timestamp; они немедленно dispatch через serialized authenticated socket channel и меняют pending на localized accepted/rejected только для matching последнего event. Server integration, reconnect physical proof и Tanbon controls остаются pending;
...
- Kerugi typed combat events wire to the durable outbox and serialized authenticated socket channel; authenticated initial/reconnect lifecycle replays due durable commands in client sequence before heartbeat and persists terminal ACK/rejection, while remaining physical action wiring, server integration proof and resync remain pending;
