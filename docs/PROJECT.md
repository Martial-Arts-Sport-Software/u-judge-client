...
- кнопки Kerugi создают durable typed commands только в authenticated `running` Kerugi session, с unique ID, monotonic client sequence, clock-adjusted timestamp и local pending feedback; realtime socket dispatch, terminal outcomes и Tanbon controls остаются pending;
