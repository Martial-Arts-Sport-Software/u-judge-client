...
- кнопки Kerugi создают durable typed commands только в authenticated `running` Kerugi session, с unique ID, monotonic client sequence, clock-adjusted timestamp и local pending feedback; matching terminal ACK/rejection from replay updates localized accepted/rejected feedback without affecting a newer event. Direct realtime socket dispatch and Tanbon controls remain pending;
