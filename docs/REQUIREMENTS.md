...
| ID      | Приоритет | Статус  | Требование                                                     | Критерий приёмки                                                            |
|---------|-----------|---------|----------------------------------------------------------------|-----------------------------------------------------------------------------|
| CLI-020 | Must      | Implemented | Клиент показывает девять дисциплинарных режимов              | Восемь дисциплин PDF 1 и продуктовая дисциплина Tanbon доступны по правилам режима |
...
| CLI-032 | Must      | Partial | Shared outbox сохраняет caller-provided unique event ID; Kerugi controls generate IDs before durable write, Tanbon pending | Два намеренных нажатия различаются, transport retry сохраняет ID       |
...
| CLI-035 | Must      | Partial | Realtime command client journals the complete command before socket send; Kerugi taps dispatch durable commands on a serialized authenticated channel; app-kill/server integration proof pending | App kill до ACK не теряет событие                                      |
| CLI-036 | Must      | Partial | Shared command transport applies terminal ACK/rejection to outbox; Kerugi renders pending/accepted/rejected feedback; server integration evidence pending | Состояние каждого недавнего события понятно судье                      |
...
| CLI-066 | Must      | Partial | Terminal command rejection is persisted and excluded from retry; Kerugi renders rejection feedback; server integration pending | Outbox отмечает final rejection и показывает действие пользователю     |
