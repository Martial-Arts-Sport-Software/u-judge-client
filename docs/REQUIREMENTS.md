...
| CLI-016 | Must      | Partial | UI polls typed HTTP pairing status and displays local pending/accepted/rejected outcomes without granting online access; realtime status wiring and reconnect remain pending | Судья не попадает на рабочий экран до accepted                                        |
